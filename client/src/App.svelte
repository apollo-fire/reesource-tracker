<script lang="ts">
    ...
    let isMaintainerOrAdmin = $derived(
        currentUserRoles.includes('maintainer') || currentUserRoles.includes('admin'),
    );
...
<main
    ...
    <div class="absolute top-2 right-4 flex items-center gap-3">
        {#if currentUserName}
            <span class="text-sm text-muted-foreground">{currentUserName}</span>
        {/if}
        {#if currentUserRoles.length > 0}
            <span class="text-xs text-muted-foreground opacity-60">{currentUserRoles.join(', ')}</span>
        {/if}
