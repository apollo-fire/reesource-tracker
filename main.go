...
func main() {
	...
	if err := auth.Initialize(ctx,
		cfg.OIDC.IssuerURL, cfg.OIDC.ClientID, cfg.OIDC.ClientSecret,
		cfg.BaseURL, cfg.OIDC.RoleClaimPath, cfg.OIDC.RoleMap,
	); err != nil {
		log.Fatal("Failed to initialise OIDC:", err.Error())
	}
	api.Routes(r)
